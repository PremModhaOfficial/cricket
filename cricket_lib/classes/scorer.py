...
def handel_byes_ball(ball_info, player_ran: Player, runs):
    player_ran.update_runs(int(runs) + 1, ball_info)


def handel_no_ball(ball_info, player_ran: Player, runs):
    player_ran.update_runs(int(runs) + 1, ball_info)


def handel_wide_ball(ball_info, player_ran: Player, runs):
    player_ran.update_runs(int(runs) + 1, ball_info)
...
class Scorer(object):
    ...
    def update_runs(self, runs):
        self.batting_team.update_team_runs(int(runs))
        if self.batting_team.runs >= self.bowling_team.runs and self.first_team_played:
            # winning
            print(f'batting team: {self.batting_team.name} won the match by {11 - self.batting_team.wicket} wickets')
            # exit(0)
    ...
    def match_reader(self, score=score_parcer.score):
        ...
        for index, command in enumerate(commands):
            ...
            if "END" in command:
                # wining
                print(f"{self.batting_team.runs} {self.bowling_team.runs}")
                print(f"{self.batting_team.name} has won by {self.bowling_team.runs - self.batting_team.runs} runs")
                # exit(0)

            if "ONE_BALL" in command:
                ball = include("ONE_BALL $ ", command)

                ball_info = ball.strip()
            ...
            if "OVER" in command:
                ...
                for ball_info in over_commands.split(" $ "):
                    ...
                    player_ran: Player = self.get_batsman_by_name(name_of_player)
                    if player_ran is None and current_ball_type != BallType.WICKET:
                        raise Exception(f"Could not find player {name_of_player}\n"
                                        f"wrong name or already out!")
                    ...
                    if current_ball_type == BallType.WICKET:
                        ...
                        self.handel_wicket(ball_info, runs)
                        continue
                self.prepare_plot_data(over_for_plots)
                continue

    def prepare_plot_data(self, current_over_runs):
        current_over_runs = current_over_runs.strip().split(" ")
        self.batting_team.over_info.append(current_over_runs)
        try:
            total_this_over = reduce(
                lambda x, y: x + y,
                map(lambda x: int(x),
                    current_over_runs)
            )
            self.batting_team.runs_per_over.append(total_this_over)
        except ValueError as e:
            print(current_over_runs, e)